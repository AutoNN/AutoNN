...
class Final:
    ...
    def get_all_best_models(self):
        m = multiple.Multiple_Model_Gen_V3(self._train_x, self._train_y, self._test_x, self._test_y, self._epochs, self._batch_size, input_shape = self._input_shape, 
                                   max_no_layers = self._max_no_layers, model_per_batch = self._model_per_batch, 
                                   save_dir = "/home/anish/ASC_ML_test_weights/")
        m.get_best_models(save = False)
        opt = model_opt.Model_Optimization(self._train_x, self._train_y, self._test_x, self._test_y, 200, m.evaluate_dict_list, save_dir = "/home/anish/ASC_ML_test_weights/candidate_models/")
        opt.optimize_models(save=True)
        stacked_models = model_stacking.Model_Stacking(self._train_x, self._train_y, self._test_x, self._test_y, opt.saved_paths, opt.model_confs, save_dir = "/home/anish/ASC_ML_test_weights/stacked_models/")
